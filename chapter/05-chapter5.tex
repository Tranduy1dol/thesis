...
\section{Đánh giá hiệu
năng}\label{ux111uxe1nh-giuxe1-hiux1ec7u-nux103ng}

\subsection{Kết quả đo đạc}\label{kux1ebft-quux1ea3-ux111o-ux111ux1ea1c}
...
\caption{Kết quả đo hiệu năng hệ thống Curve1024}\tabularnewline
\toprule\noalign{}
...
\subsection{Phân tích: Kết quả có hợp lý về mặt lý thuyết
...
\subsection{Tính khả thi trong ứng dụng thực
...
\section{Đánh giá độ an
toàn}\label{ux111uxe1nh-giuxe1-ux111ux1ed9-an-touxe0n}

\subsection{An toàn trước tấn công cổ
...
\(\rho\)} \citep{PollardRho1978}, có độ phức tạp
\(O(\sqrt{r}) \approx O(2^{256})\) phép toán nhóm. Với \(2^{256}\) bước
tính toán cần thực hiện, ngay cả khi huy động toàn bộ năng lực tính toán
của nhân loại --- mỗi nguyên tử Trái Đất thực hiện \(10^9\) phép tính
mỗi giây từ khi Big Bang --- vẫn không đủ để phá vỡ khóa trong tuổi thọ
của vũ trụ. Đây là \textbf{biên an toàn tuyệt đối} trước mọi hệ thống
máy tính cổ điển.
...
\subsection{An toàn trước tấn công Invalid
...
\subsection{An toàn trước tấn công
MOV}\label{an-touxe0n-trux1b0ux1edbc-tux1ea5n-cuxf4ng-mov}

\textbf{Tấn công MOV} (Menezes-Okamoto-Vanstone) \citep{MOV1993} sử dụng
phép ghép cặp (pairing) để ánh xạ bài toán ECDLP trong
\(E(\mathbb{F}_p)\) về bài toán DLP trong trường hữu hạn mở rộng
\(\mathbb{F}_{p^k}\), nơi bài toán DLP có thể được giải hiệu quả hơn
bằng thuật toán chỉ số (index calculus).
...
bit để kháng tấn công này ở mức 128-bit \citep{BarbulescuDuquesne2019};
hệ thống đạt \(18 \times 1024 = 18432\) bit, vượt xa tiêu chuẩn này. Kết
quả này được test \texttt{test\_mov\_attack\_resistance} xác nhận tường
minh.

\subsection{An toàn trước tấn công
Anomalous}\label{an-touxe0n-trux1b0ux1edbc-tux1ea5n-cuxf4ng-anomalous}

\textbf{Tấn công Anomalous (SSSA)} \citep{SmartAnomaly1999} khai thác
trường hợp đặc biệt khi \(\#E(\mathbb{F}_p) = p\), tức là số điểm trên
đường cong bằng đúng modulus. Trong trường hợp này, tồn tại một đẳng cấu
nhóm ánh xạ bài toán ECDLP về bài toán trên nhóm cộng
\(\mathbb{Z}/p\mathbb{Z}\), cho phép giải trong thời gian tuyến tính.
...
\subsection{Hạn chế: Timing Side-Channel
