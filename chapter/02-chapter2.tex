...
\addcontentsline{toc}{chapter}{Cơ sở toán học}
...
\section{\texorpdfstring{Trường hữu hạn
\(\mathbb{F}_p\)}{Trường hữu hạn \textbackslash mathbb\{F\}\_p}}\label{trux1b0ux1eddng-hux1eefu-hux1ea1n-mathbbf_p}
...
\KeywordTok{pub} \KeywordTok{trait}\NormalTok{ PrimeFieldConfig }\OperatorTok{\{}
    ...
    \KeywordTok{const}\NormalTok{ N\_PRIME}\OperatorTok{:}\NormalTok{ U1024}\OperatorTok{;}  \CommentTok{// {-}p⁻¹ mod 2\^{}1024, dùng cho REDC}
\OperatorTok{\}}
...
kiện. Chi phí: \(O(n)\) với \(n\) là số limb (16 limb × 64 bit = 1024
bit).
...
\subsection{Định nghĩa}\label{ux111ux1ecbnh-nghux129a-1}
...
\KeywordTok{pub} \KeywordTok{fn}\NormalTok{ pow(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ exp}\OperatorTok{:}\NormalTok{ U1024) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{Self} \OperatorTok{\{}
    ...
    \KeywordTok{let} \KeywordTok{mut}\NormalTok{ base }\OperatorTok{=} \OperatorTok{*}\KeywordTok{self}\OperatorTok{;}
    \ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in} \DecValTok{0}\OperatorTok{..}\DecValTok{16} \OperatorTok{\{}           \CommentTok{// 16 limb × 64 bit = 1024 bit}
