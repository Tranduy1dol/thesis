...
\textbf{Contributions.} This paper addresses both concerns:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  We modify the Cocks-Pinch construction to \emph{guarantee} high
  two-adicity in both field primes by constraining the cyclotomic seed
  \(T\). Specifically, choosing \(T \equiv 0 \pmod{2^{12}}\) yields
  \(\nu_2(r-1) \geq 36\) deterministically, while an extended lift
  search ensures \(\nu_2(p-1) \geq 36\). The resulting two-adicity of 36
  exceeds BLS12-381's value of 32.
\item
  We introduce a \emph{readability-aware prime selection} framework that
  scores candidate primes on eight structural criteria (binary
  sparseness, zero limbs, hex density, etc.) and selects the most
  human-readable valid parameters without sacrificing security.
\end{enumerate}

We instantiate these techniques to produce \emph{Curve1024}: a
pairing-friendly curve over a 1024-bit prime field with embedding degree
\(k=18\), scalar field order \(r \approx 2^{512}\) (256-bit ECDLP
security), and NTT support up to length \(2^{36}\) in both fields. A
complete Rust implementation validates correctness through 36 tests and
demonstrates practical digital signature operations.
...
system. Section 5 reports implementation results and security analysis.
